...
def recInfoHandle(request):
    ...
    if request.method == 'POST':
        ...
        if name == '':
            context = {"user_errinfo": "收件人不能为空！"}
        elif tel == '':
            context = {"tel_errinfo": "电话不能为空！"}
        elif address == "":
            context = {"add_errinfo": "地址不能为空"}
        else:
            recinfo = RecInfo()
            recinfo.name = name
